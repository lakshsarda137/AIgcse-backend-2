...
loader=DirectoryLoader('/Users/LakshSarda/PycharmProjects/pythonProject3/Computer_Science_Chatbot/Physics_chatbot/explainer_phy',loader_cls=PyPDFLoader)
docs=loader.load()
...
def physics_explainer(question,history):
    sentences=history
    ...
    You are a component of a chatbot that helps explain concepts to students in Physics specific to only the IGCSE 
    academic curriculum like a Physics teacher. You have been given a set of pdfs that explain various concepts relevant to the syllabus of Physics in IGCSE
    ...
    is not necessary that everything related to Physics is in the notes. The notes are just confined to the iGCSE
    Physics syllabus and therefore your answers must also be confined to the IGCSE Physics syllabus. Please answer
    ...
    knowledge. Your answer should be comprehensive and should enable the user to understand the answer to their question very well.
    {context}
    Question: {question}
...
def add_item():
    data = request.json
    return physics_explainer (data["question"], data["memory"]),201
