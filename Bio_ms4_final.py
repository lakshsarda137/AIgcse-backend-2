...
biology_docs=DirectoryLoader('/Users/LakshSarda/PycharmProjects/pythonProject3/Computer_Science_Chatbot/Biology_chatbot/question_papers_bio4_all',loader_cls=PyPDFLoader)
biology_documents=biology_docs.load()
text_splitter=RecursiveCharacterTextSplitter(
    chunk_size=500,
    chunk_overlap=200,
    add_start_index=True
)
split_docs=text_splitter.split_documents(biology_documents)
...
syllabus_loader=PyPDFLoader('/Users/LakshSarda/PycharmProjects/pythonProject3/Computer_Science_Chatbot/Biology_chatbot/595426-2023-2025-syllabus.pdf')
...
def bio_ms_4(question,predefined_history):
    ...
    You are a component of a chatbot specifically designated to output all content in the IGCSE biology
    Syllabus corresponding to the user's question. This is the process you should follow:
    1. Identify key words in the user's question which could include technical biology terms etc.
    ...
    You are a chatbot specifically designed to give answers to user questions in exact answer-key language in biology
    ...
    4.  If relevant documents were retrieved,  you must try to output an answer from the answer key as it is without missing out points.
        ...
        Please make sure that the answer you output is factually correct by making sure that the answer makes sense to you- a biology
         teacher.
    ...
    so.
    Place this answer under "Exact answer key content" and use this exact answer key content's every point to created a "Framed answer"
    which does not miss out on any points from the relevant exact answer key content and does not add on its own points.
...
def add_item():
    ...
    return bio_ms_4 (data["question"], data["memory"]),201
